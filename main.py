...
def draw_window(surface,grid,score=0, last_score=0):
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            pygame.draw.rect(surface, grid[i][j], (top_left_x + j * 20, top_left_y + i * 20, 20, 20), 0)
...
def draw_grid(surface, row, col):
    ...
    sy = top_left_y
    for i in range(row):
        pygame.draw.line(surface, (128, 128, 128), (sx, sy + i * 20),(sx + play_width, sy + i * 20))  # horizontal lines
        for j in range(col):
            pygame.draw.line(surface, (128, 128, 128), (sx + j * 20, sy),(sx + j * 20, sy + play_height))  # vertical lines
class Piece(object):
    rows = 20  # y
    columns = 10  # x
...
def main_menu(win):  # *
    ...
    while run:
        win.fill((0,0,255))
        draw_text_middle(win, 'Press Space to play  and Press Esc to quit', 20, (255,255,255))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
